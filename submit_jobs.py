...
parser.add_argument('-alg_name', action='store', dest='algs', default='lex_std', type=str, help='Algorithm name')
parser.add_argument('-S', action = 'store', dest='Ss', type=str, default = '100', help='Population size')
parser.add_argument('-N', action = 'store', dest='Ns', type=str, default = '100', help='N in NK landscape')
parser.add_argument('-K', action = 'store', dest='Ks', type=str, default = '8', help='K in NK landscape')
...
parser.add_argument('-seed', action='store', type=str, dest='seeds', default='26311,32400')
parser.add_argument('--snapshot', action='store', dest='snapshots', type=str, default=True, help='Take snapshots of population')
...
parser.add_argument('--slurm', action='store_true', default=True, help='Run on an slurm HPC')
...
for alg,s,n,k,seed,n_gen in it.product(algs,Ss,Ns,Ks,seeds,n_gens):
	...
	all_commands.append(
	f'python /mnt/home/shahban1/potentiation/single_nk_experiment.py -alg_name {alg} -S {int(s)} -N {int(n)} -K {int(k)} -seed {int(seed)} -rdir {rdir} -n_gen {int(n_gen)} --snapshot {snapshot}'
	)
...
if args.slurm:
	...
	for i, run_cmd in enumerate(all_commands):
		...
		if snapshot:
			out_file = job_info[i]['rdir'] + '/snapshots_out/' + job_name + '_snapshot_%J.out'
		else:
